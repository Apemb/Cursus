...
public class LinearBoardReaderTests
{
    ...
    [Fact(DisplayName = "étant donné une issue étiquetée, quand on lit la page, alors elle porte ses étiquettes")]
    public void An_issue_carries_its_labels()
    {
        ...
        const string json = """
            {"data":{"issues":{"pageInfo":{"hasNextPage":false},"nodes":[
              {"identifier":"CUR-12","title":"ReadTask contre l'API réelle","state":{"name":"Todo"},
               "parent":null,"project":{"id":"p-1"},
               "labels":{"nodes":[{"name":"Feature"},{"name":"cursus:ready"}]}}
            ]}}}
            """;
        ...
        // assert
        Assert.Equal(["Feature", "cursus:ready"], Assert.Single(page.Items).Labels);
    }
    ...
    public void An_issue_without_the_labels_field_carries_none()
    {
        ...
        const string json = """
            {"data":{"issues":{"pageInfo":{"hasNextPage":false},"nodes":[
              {"identifier":"CUR-12","title":"ReadTask","state":{"name":"Todo"},
               "parent":null,"project":{"id":"p-1"}}
            ]}}}
            """;
        ...
        // assert
        Assert.Empty(Assert.Single(page.Items).Labels);
    }
