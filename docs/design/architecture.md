...
| `ScriptStep.cs` · `AgentStep.cs` | Les deux kinds. `ScriptStep` porte `Script` (`ScriptSpec`) · `AgentStep` porte `HarnessName`, `ModelId`, `Prompt` (identifiants ; l'invocation vit dans l'exécuteur) |
...
| `IStepExecutor.cs` · `ScriptStepExecutor.cs` · `AgentStepExecutor.cs` | L'exécution d'une étape, **par type** : `CanExecute(step)` + `ExecuteAsync(step, cwd, flux)` → `ScriptResult`. Le script enveloppe l'`IProcessRunner` ; l'agent bâtit `claude --model … -p …` headless (même runner). Le moteur choisit l'exécuteur, kind-aveugle. Voir §5, `D-030` |
...
| `WorkflowValidator.cs` · `ValidationReport.cs` | Validité du graphe · `ValidationIssueKind` (11 valeurs — dont `EmptyAgentPrompt`/`UnknownAgentModel`, `D-030`), `ValidationIssue`, `ValidationReport` |
...
La résolution `context.Resolve(step.WorkingSubdirectory)`, juste avant l'appel à l'exécuteur, est le **seul endroit du système** où un sous-chemin relatif devient un répertoire absolu : le moteur est le seul à connaître le contexte, donc le seul à pouvoir traduire — il passe le cwd absolu à l'exécuteur, qui l'applique à sa façon (le `ScriptStepExecutor` par un `with` non destructif sur le `ScriptSpec`, `D-030`).
...
| `Workflows/WorkflowSerializerTests.cs` (20) | L'aller-retour caractère pour caractère (`Read`→`Write`) et l'idempotence (`Write`→`Read`→`Write`), le document servant de référentiel de comparaison ; les formes de malformation ; « absence de timeout ≠ zéro » ; la **description** portée, absente, et survivant à l'aller-retour (`D-018`) ; la porte **`ReadEditable`** qui rend un graphe invalide malgré tout et annule un document malformé (`D-020`) ; le kind **`agent`** relu, l'aller-retour d'un `AgentStep` (harness/modèle/prompt), la retombée `kind` absent → script (`D-030`). |
| `Workflows/RunContextTests.cs` (10) | Les **justifications absentes du code** du refus d'une racine relative ou inexistante. |
| `Workflows/WorkflowValidatorTests.cs` (13) | La motivation de chaque règle et l'**ordre exact** d'un rapport multi-issues ; les deux règles propres à l'agent — prompt vide (`EmptyAgentPrompt`) et modèle hors harness ou harness inconnu (`UnknownAgentModel`), `D-030`. |
...
Le pari central promet que greffer un nouveau type d'étape sera une extension, pas une refonte. L'**`AgentStep` l'a désormais parcourue** (`D-030`) : la recette n'est plus un contrat théorique, elle a été suivie une fois pour de bon. Ce qui suit décrit donc à la fois **ce qui a été fait** et le contrat que le prochain kind (`TaskStep`) devra tenir.

> **Deux kinds construits, un prévu — et l'ordre a été inversé** : `ScriptStep` et **`AgentStep`** (`D-030`) existent ; **`TaskStep`** (§7.10) reste. L'agent est passé **devant** `TaskStep`, contrairement à ce que cette section annonçait : `TaskStep` dépend d'un client tracker (Linear) que la trajectoire a écarté d'ouvrir en premier, là où l'agent ne dépend que du noyau déterministe déjà là. `TaskStep` reste le cobaye idéal du *prochain* passage — synchrone, résultat binaire, sans PTY — mais il a désormais l'`AgentStep` comme précédent construit.
