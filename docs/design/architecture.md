...
graph TD
    subgraph CoreLib["Cursus.Core"]
        ...
        Workflows["Workflows/<br/><i>racine : vocabulaire (graphe, run, script)</i><br/>Execution · Projection · Serialization · Validation<br/>Journaling · Output · Workspaces · Editing"]
...
dotnet test                           # attendu : 332 verts (chiffre de référence de ce document)
...
La moitié **workflow** est désormais câblée à l'UI (jonction 6c close — l'App charge, valide, lance, visualise et annule un `WorkflowRun` ; §1.1, §4.16–4.18). Le trou qui subsiste est l'**autre** couture : un run ne s'adosse à **aucune session terminal vivante** — workflow ↔ **sessions/PTY**, ce que décrit cette section.

Aucun fichier de `Workflows/` ne mentionne `Sessions`, `TerminalSession` ou `SessionWorkspace` : le noyau d'orchestration ignore toujours la moitié sessions. En sens inverse, `Cursus.App` **pilote** bien le workflow — `ProjectHost.LaunchAsync` instancie `WorkflowEngine` via `WorkflowLauncher` (§4.16) — mais **aucun run ne s'ouvre dans un PTY** : les deux mondes s'exécutent encore par des mécanismes distincts qui s'ignorent (§2.1).

Concrètement :

- **Un run ne s'ouvre dans aucune session terminal** : l'écran de run montre le log *coulé* du puits d'artefact (tail, §4.17), pas un PTY interactif ; lancer un workflow n'ouvre pas d'onglet terminal.
...
### 2.2 Comment les recoudre — TRANCHÉ pour le headless, PTY encore ouvert

Deux coutures étaient structurellement disponibles ; la seconde est **tranchée et construite** (`D-030`) :

1. **Un `PtyProcessRunner` derrière `IProcessRunner`.** `IProcessRunner` est la seule couture d'exécution du noyau : y brancher RoyalTerminal ne toucherait pas une ligne de `WorkflowEngine`. Mais cela force un PTY interactif dans un contrat conçu pour du non-interactif — pas de code de sortie fiable sans convention supplémentaire, sortie qui n'est pas « rendue » mais « coulée ». **Reportée avec la session PTY** (« plus tard indéterminé »).
2. **Un `StepKind` distinct (`AgentStep`) avec son propre exécuteur** — le chemin retenu. L'`AgentStep` est un sous-type de `StepDefinition` (§5), son `AgentStepExecutor` réalise `IStepExecutor`, et le moteur route par le *type* de l'étape. C'est le chemin annoncé par le pari central (§3) : chaque monde garde son contrat, et la boucle de traversée n'a pas bougé.

**La question qui manquait pour trancher est résolue** : un `AgentStep` **headless** rend bien un `ScriptResult` (`claude --model … -p …`, tubes redirigés, code de sortie), donc il est routable par les gardes existantes **sans généraliser `Guard`**. La forme « résultat d'une autre nature » ne se rouvrira, s'il le faut, qu'avec la session PTY interactive.
...
Namespace racine `Cursus.Core.Workflows` pour le **vocabulaire partagé**, plus **huit sous-namespaces** pour les services (ci-dessous), et `Cursus.Core.Projects` pour ce qui l'ancre sur un disque (§4.11). Le code est court et commenté : cette section donne la carte, l'artefact utilisateur, et ce qui n'est **pas** déductible d'une lecture.
...
| `…Execution` | `WorkflowEngine`, `WorkflowLauncher`, `RunContext`, `StepExecutionContext`, `IClock`, `IProcessRunner`, `ProcessRunner`, `PathStrategy`, `PathEscapesWorkspaceException`, `ITaskTracker`, `TaskCard`, `TaskStepExecutor` |
...
| `…Editing` | `WorkflowDraft` — surface d'édition mutable qui **construit** (`AddStep`/`AddAgentStep`, `SetEntryStep`, `SetScript`, `SetPrompt`/`SetModel`, `AddEdge`/`RemoveEdge`) et **remanie** (rename/remove) un graphe, tenant l'**unicité d'id** et la clôture référentielle (§4.19–4.20, `D-020`/`D-021`/`D-031`) ; `Slug` (libellé → id), `ArgumentLine` (ligne d'arguments ⟷ argv, à guillemets, `D-024`), `CommandLine` (ligne de commande ⟷ binaire + arguments, 1er token = binaire, `D-029`), `DuplicateStepIdException` |
...
| `StepDefinition.cs` | Un nœud, **abstrait** : le commun `Id`, `Name` (**titre court**), `MaxVisits`, `OutEdges`, `WorkingSubdirectory?` (relatif), `Description?`. Ce que l'étape *fait* descend dans les sous-types (`D-030`) |
| `ScriptStep.cs` · `AgentStep.cs` · `TaskStep.cs` · `TaskOperation.cs` | Les trois kinds. `ScriptStep` porte `Script` (`ScriptSpec`) · `AgentStep` porte `HarnessName`, `ModelId`, `Prompt` (identifiants ; l'invocation vit dans l'exécuteur) · `TaskStep` porte une `TaskOperation` — `abstract record` + `ReadTask`/`MoveCard(Column)`/`ApplyLabel(Label)`, variante de *type* (`D-032`) ; la tâche visée n'est pas dans la définition mais dans le `RunTrigger` du run |
| `AgenticHarness.cs` · `AgentModel.cs` | Le harness comme concept nommé : `Name` (« Claude Code ») + liste de `Models` (`Id`, `Label`). Catalogue racine (`AgenticHarness.ClaudeCode`), donnée pure. Voir `D-030` |
...
| `IStepExecutor.cs` · `ScriptStepExecutor.cs` · `AgentStepExecutor.cs` · `TaskStepExecutor.cs` | L'exécution d'une étape, **par type** : `CanExecute(step)` + `ExecuteAsync(step, StepExecutionContext, flux)` → `ScriptResult`. Le script enveloppe l'`IProcessRunner` ; l'agent bâtit `claude --model … -p …` headless (même runner) ; la tâche agit sur le tableau via `ITaskTracker` et rend un échec routable sans clé ou sur faute (`D-032`). Le moteur choisit l'exécuteur, kind-aveugle, et passe un `StepExecutionContext` (cwd résolu + clé de tâche). Voir §5, `D-030`, `D-032` |
...
| `WorkflowValidator.cs` · `ValidationReport.cs` | Validité du graphe · `ValidationIssueKind` (13 valeurs — dont `EmptyAgentPrompt`/`UnknownAgentModel`, `D-030` ; `EmptyTaskMoveColumn`/`EmptyTaskLabel`, `D-032`), `ValidationIssue`, `ValidationReport` |
...
| `Editing/WorkflowDraft.cs` | Surface d'édition mutable. **Construire** : `AddStep`/`AddAgentStep` (slug + désambiguïse → id unique ; l'agent naît harness Claude Code + 1er modèle + prompt vide, `D-031`), `SetEntryStep`, `SetScript`, `SetPrompt`/`SetModel` (portes sœurs par kind, cast garde l'invariant, `D-031`), `AddEdge`/`RemoveEdge`. **Remanier** : `RenameStep` (retarge + refuse une collision), `RemoveStep` (purge). `new(def)`/`ToDefinition()`. Invariants : unicité d'id tenue, validité tolérée (sujet doit exister, référence peut pendre). Voir §4.19–4.20, `D-020`/`D-021`/`D-031` |
...
| `Editing/CommandLine.cs` | Ligne de **commande** humaine ⟷ `(FileName, Arguments)` : le **1er token est le binaire**, le reste ses arguments. Posé **sur** `ArgumentLine` (tokeniseur + requoteur réutilisés). Vide toléré (`Format("", []) → ""`), round-trip `Format∘Parse = id`. Alimente le champ « Commande » unique de l'éditeur. Voir `D-029` |
...
`WorkflowEngine` : classe scellée, `ctor(IProcessRunner, IRunJournal, IRunOutputStore, ITaskTracker? = null)` — le tracker est optionnel tant que le client réel n'est pas branché (`D-032`, défaut = null-object qui refuse tout geste, l'étape-tâche échoue alors routablement) —, une seule méthode publique
...
flowchart TD
    ...
    D -->|non| E["wd = context.Resolve(step.WorkingSubdirectory)<br/>executor = _executors.First(e =&gt; e.CanExecute(step))"]
    E -->|évasion| EX(["PathEscapesWorkspaceException<br/>remonte"])
    E --> F["await executor.ExecuteAsync(step, wd, sink.Stdout, sink.Stderr, ct)"]
...
La résolution `context.Resolve(step.WorkingSubdirectory)`, juste avant l'appel à l'exécuteur, est le **seul endroit du système** où un sous-chemin relatif devient un répertoire absolu : le moteur est le seul à connaître le contexte, donc le seul à pouvoir traduire — il en bâtit un `StepExecutionContext` (cwd absolu + clé de la tâche du run, `D-032`) qu'il passe à l'exécuteur, lequel l'applique à sa façon (le `ScriptStepExecutor` par un `with` non destructif sur le `ScriptSpec`, `D-030` ; le `TaskStepExecutor` en lisant la clé).
...
8. **Le moteur ne connaît que `StepDefinition` + une liste d'`IStepExecutor` + `IRunJournal` + `IRunOutputStore`.** Il route sur le *type* de l'étape (`CanExecute`) et délègue l'exécution — il ne connaît **aucun kind** (ni script, ni agent, ni tâche), et sa boucle n'a pas bougé en accueillant l'`AgentStep` (`D-030`) puis le `TaskStep` (`D-032`). C'est le pari central du pivot ; §5 en donne le garde-fou vérifiable. Les exécuteurs, eux, tiennent leurs collaborateurs (`IProcessRunner` pour script et agent ; le port `ITaskTracker` pour le `TaskStep`, `D-032`). Le journal n'y déroge pas : le moteur *émet*, il ne *lit* jamais — d'où deux interfaces séparées (§4.10). Le **provisionnement du workspace n'y entre pas** (jalon 6b, §4.13) : c'est un collaborateur de l'**appelant**, qui provisionne le worktree *avant* `ExecuteAsync` et le démonte *après*. Corollaire tranché au 6b : **l'identité du run est une entrée**, plus un `Guid` forgé dans le moteur — l'appelant la connaît en amont, puisque c'est lui qui monte le worktree à ce nom.
9. **Aucune donnée ne circule d'une étape à l'autre** : aucune sortie de `StepRun` n'alimente le `ScriptSpec` suivant, il n'existe pas encore de câblage de références. La seule mémoire partagée d'un run est **le système de fichiers du workspace** — depuis le jalon 6b, le **worktree isolé** du run (§4.13) ; c'est par lui que le `TaskStep.ReadTask` fait parvenir le corps d'une tâche à l'`AgentStep` en aval (`TASK.md`, `D-032`), sans qu'aucune référence n'ait à circuler. Une **donnée de run**, elle, descend désormais du moteur vers l'exécuteur par le **`StepExecutionContext`** (`D-032`) : aujourd'hui la clé de tâche du `RunTrigger`, demain la porte d'entrée des références. Le câblage par références façon Conductor (`${taskRef.output.champ}`) est relevé dans `landscape.md` comme vocabulaire à emprunter : **reporté, non écarté** — son véhicule (`StepExecutionContext`) est posé, son contenu (les sorties des étapes précédentes) reste à câbler. Il faudra le rouvrir quand un prompt d'`AgentStep` voudra dépendre de la sortie *structurée* d'une étape précédente, au-delà de ce que le fichier partagé porte. L'état durable *entre* workflows, lui, ne sera **pas** un magasin Cursus : il vit dans les systèmes de référence — git (branche, PR) et le tracker (issue properties Jira, kv idempotent) — modèle Symphony « re-dériver du tracker + filesystem ».
...
| `Workflows/AgentStepExecutorTests.cs` (2) · `Workflows/AgentStepEngineTests.cs` (2) | Le kind agent au moteur (`D-030`) : l'exécuteur reconnaît un `AgentStep` (`CanExecute`) et l'invoque en headless (`claude --model … -p …` sur le runner) · le moteur **route par type** — une étape-agent est confiée à l'exécuteur d'agent, puis portée par les gardes existantes vers la cible, exactement comme une étape-script (le pari central du pivot). |
| `Workflows/TaskStepExecutorTests.cs` (5) | Le kind tâche (`D-032`) : sur un `StubTaskTracker`, l'exécuteur **déplace** la carte du contexte, **pose** une étiquette, **lit** la tâche et en écrit le corps dans `TASK.md` du worktree ; sans clé de tâche il n'agit pas et **échoue routablement** ; un tracker qui lève devient un échec, pas une exception qui remonte. |
| `Workflows/WorkflowSerializerTests.cs` (23) | L'aller-retour caractère pour caractère (`Read`→`Write`) et l'idempotence (`Write`→`Read`→`Write`), le document servant de référentiel de comparaison ; les formes de malformation ; « absence de timeout ≠ zéro » ; la **description** portée, absente, et survivant à l'aller-retour (`D-018`) ; la porte **`ReadEditable`** qui rend un graphe invalide malgré tout et annule un document malformé (`D-020`) ; le kind **`agent`** relu, l'aller-retour d'un `AgentStep` (harness/modèle/prompt), la retombée `kind` absent → script (`D-030`) ; le kind **`task`** relu par opération — `MoveCard`/`ApplyLabel`/`ReadTask` retrouvés via le discriminant `operation` (`D-032`). |
| `Workflows/RunContextTests.cs` (10) | Les **justifications absentes du code** du refus d'une racine relative ou inexistante. |
| `Workflows/WorkflowValidatorTests.cs` (15) | La motivation de chaque règle et l'**ordre exact** d'un rapport multi-issues ; les deux règles propres à l'agent — prompt vide (`EmptyAgentPrompt`) et modèle hors harness ou harness inconnu (`UnknownAgentModel`), `D-030` ; les deux règles propres à la tâche — déplacement sans colonne (`EmptyTaskMoveColumn`) et étiquette sans nom (`EmptyTaskLabel`), `D-032`. |
| `Workflows/Editing/WorkflowDraftTests.cs` (23) | Les invariants du brouillon. *Remanier* (`D-020`) : aller-retour neutre, renommer qui retarge + fait suivre l'entrée, supprimer qui purge + vide l'entrée. *Construire* (`D-021`) : ajouter qui slugifie et **désambiguïse** un id pris, poser l'entrée (permissif sur un fantôme), affecter un script (lève sur un id absent), tracer/retirer une arête (sujet doit exister, cible permise absente), renommer vers un id pris **refusé**. *Construire un agent* (`D-031`) : `AddAgentStep` qui sème harness Claude Code + 1er modèle + prompt vide, `SetPrompt`/`SetModel` (lèvent sur un id absent, choke `IndexOf` partagé). |
...
| `Workflows/Editing/CommandLineTests.cs` (12) | Le découpage binaire / arguments : tête = binaire, reste = args (délégué à `ArgumentLine`), token unique, ligne vide tolérée, arg et binaire cités ; `Format` qui préfixe le binaire (vide → chaîne vide) ; la **propriété de round-trip** `Format∘Parse = id` (`D-029`). |
...
Le moteur **raconte** désormais ce qu'il fait. `WorkflowEngine(IProcessRunner, IRunJournal, IRunOutputStore, ITaskTracker? = null)` : le journal **et** le puits de sortie (§4.12, jalon 6a) sont des paramètres **obligatoires**, jamais optionnels — un défaut muet rendrait le silence accidentel, alors que c'est précisément le trou qu'on referme ; un run qu'on ne veut pas relire prend simplement un `InMemoryRunJournal` et un `InMemoryRunOutputStore` qu'on ignore. Le **tracker**, lui, est **délibérément optionnel** (`D-032`) : c'est une capacité qui n'existe pas encore côté production (le client Linear arrive en 2·2b), et son absence n'est pas un silence accidentel mais un état nommé — le null-object refuse tout geste, l'étape-tâche échoue routablement. Depuis le jalon 6b, `Append` est **sérialisé par un `lock`** dans les deux implémentations : plusieurs runs d'un même projet écrivent sur la même connexion, qui n'est pas thread-safe (§4.13).
...
- **La sortie ruisselle *vraiment*, sans flush de l'appelant** (jambe 1, `D-028`) : `LazyAppendStream` pousse son buffer managé vers le handle OS (`Flush(flushToDisk: false)`) **après chaque écriture**, si bien qu'un suiveur (`ArtifactTail`, autre handle) voit la sortie en direct et non d'un bloc à la clôture. `flushToDisk: false` : on veut la visibilité inter-handle, pas la durabilité — pas de fsync par ligne de sortie de test. C'est ce qui fait tenir la promesse du jalon 6a *au réel*, celle que le flush-à-la-seule-clôture démentait en pratique.
...
Quatrième pierre de l'arc **authoring**. `D-020` (·2b) avait donné au brouillon de quoi *remanier* un
graphe ; il ne savait pas le *construire* depuis rien. `D-021` livre cette surface — toute en Core,
testable, sans une ligne d'UI.

- **`Slug` (`Workflows/Editing/`) — libellé humain → identifiant sûr.** Fonction pure : minuscules,
  **dépliage des diacritiques** (décomposition `FormD` + rejet des marques non-espaçantes — « Générer » →
  « generer », crucial en projet francophone), suites de blancs → un tiret, rejet de tout ce qui n'est pas
  `[a-z0-9-]`, fusion/rognage des tirets. Réutilisé deux fois : l'id d'étape (par `AddStep`) et l'id de
  *fichier* d'un workflow (par la VM de ·3b) — où le rejet des séparateurs le rend légal au regard de
  `PathOf` (`D-019`), la garde `InvalidWorkflowIdException` n'ayant alors plus rien à rejeter.

- **`WorkflowDraft` — cinq opérations de construction.** `AddStep(name)` fait naître une étape (`Name` =
  libellé, `Id` = slug **désambiguïsé** par suffixe, script vide, une visite, aucune arête) et **retourne
  l'id** ; `SetEntryStep`, `SetScript`, `AddEdge`, `RemoveEdge` (par index) complètent. La garde de
  `RenameStep` contre une collision d'id est ajoutée (`DuplicateStepIdException`).

- **Deux régimes d'invariant, le nerf de `D-021`.** L'**unicité d'id** est *tenue* : les opérations du
  brouillon travaillent par id, deux étapes sous un même id les rendraient indéfinies — d'où `AddStep` qui
  désambiguïse et `RenameStep` qui refuse. La **validité du graphe** reste *tolérée* : entrée non posée,
  cible fantôme, script vide passent, le validateur les rapporte (« brouillons permis »). D'où l'asymétrie
  des gardes : le **sujet** d'une opération doit exister (`SetScript`/`AddEdge` lèvent `UnknownStepException`),
  sa **référence** peut pendre (`SetEntryStep`, la cible d'`AddEdge`). L'asymétrie *désambiguïse vs refuse*
...
- **Le module éditeur (App, `D-023`).** `WorkflowEditorViewModel` est un adaptateur mince sur un
  ...
  **valide en direct** (`WorkflowValidator.Validate(draft.ToDefinition())`). C'est le **troisième module**
  ...
  brouillon ; le VM ne fait que binder et déléguer. Le **champ de commande** d'une étape est **local** et
  descend seul dans le brouillon à la perte de focus (sans re-projeter, pour ne pas recréer la ligne
...
### 4.24 Le tableau en entier : la lecture paginée — CONSTRUIT (`CUR-45`, `D-040`)

Le client Linear montrait la **première page** du tableau et l'avouait. Il le montre désormais en
entier. Le changement est confiné à `Cursus.Trackers` — `ITaskBoard` n'a pas bougé, ce qui est le
signe qu'il était à la bonne place.

**Deux requêtes, pas une renversée.** La forme d'origine (`projects { issues }`) portait un curseur
**par projet** : la paginer aurait demandé N+1 boucles imbriquées. Linear expose `issues` **à la
racine**, avec un curseur unique et le projet porté par chaque issue. Mais ce renversement seul
perdrait les projets **vides** — aucun ne serait nommé par aucune issue. D'où deux requêtes :

| Requête | Ce qu'elle apporte | Coût mesuré |
|---|---|---|
| `projects(250) { id name }` | tous les projets, **vides compris** | 600 |
| `issues(250, after:) { … project { id } }` | toutes les cartes, **un seul curseur** | 8 |

**608 pour le tableau entier**, contre **8 280** pour l'ancienne forme — qui vivait donc à 83 % du
plafond de complexité par requête (10 000). Voir `docs/reference/linear-api.md` §6a et §7bis.

| Type | Rôle |
|---|---|
| `LinearBoardCollector` | la **boucle** : suit le curseur jusqu'à épuisement, pour les deux connexions, puis assemble. Reçoit son transport en **délégué** — donc testé sans réseau |
| `LinearPage<T>` | les éléments d'une page et le curseur de la suivante. Générique parce que toutes les connexions de cette API ont la même forme |
| `BareProject` | un projet sans ses issues |
| `FlatIssue` | inchangée dans son rôle ; gagne `ProjectId`, sort de sa classe hôte |
| `LinearBoardReader` | gagne `ReadProjects` / `ReadIssues` / `Assemble` ; **perd** `Read` et la forme imbriquée |

**Pourquoi la boucle n'est pas dans l'adaptateur HTTP.** `LinearTaskBoard` est mince et non testé
parce qu'il ne décide rien. Une boucle de pagination décide — et casse en **silence** : curseur non
transmis, dernière page perdue, arrêt jamais atteint, aucun des trois ne lève d'exception. Elle vit
donc là où un test peut lui fournir un faux transport, qui **retient les requêtes** : c'est la seule
façon de prouver qu'un curseur a réellement voyagé, et non que le double a avancé de lui-même.

⚠️ **Le piège qui justifie cette place** : Linear rend un `endCursor` **plein sur la dernière page**,
alors que `hasNextPage` est faux. Lire le curseur sans cette garde offrirait une page suivante là où
il n'y en a pas, et la boucle redemanderait éternellement la même. `hasNextPage` décide, jamais la
présence du curseur.

**`TaskProject.IsTruncated` est supprimé** — du domaine, de la vue-modèle et du bandeau XAML. Un aveu
qui ne peut plus être vrai est un mensonge inverse, et un champ mort induit le prochain lecteur en
erreur. Il n'y a **aucun plafond de pages** : décision du rôle produit, un tableau montré à moitié est
faux quelle que soit la suite, tandis que la lenteur se mesure après. La seule protection contre
l'infini est une **garde de non-progression** (on s'arrête si l'API renvoie le curseur avec lequel on
vient de demander).

**Trois trous connus, actés plutôt que découverts plus tard :**

- une issue dont le `ProjectId` n'est dans aucun projet lu est **perdue sans bruit** — les deux listes
  viennent de deux requêtes successives, donc une carte créée entre les deux y tombe ;
...
**Et un défaut trouvé par la sonde, hors périmètre** : l'API rend les noms **HTML-échappés**
(`visuel &amp; configuration`). L'enjeu dépasse l'affichage — une étiquette contenant `&` ferait
échouer un prédicat de déclenchement sans qu'aucune erreur ne s'affiche. `CUR-46`.

---

## 5. Ajouter un StepKind : la recette

Le pari central promet que greffer un nouveau type d'étape sera une extension, pas une refonte. L'**`AgentStep` l'a parcourue** (`D-030`), puis le **`TaskStep` l'a parcourue à son tour** (`D-032`, jambe 2·2a) : la recette n'est plus un contrat théorique, elle a été suivie **deux fois** pour de bon, sans que la boucle de traversée bouge. Ce qui suit décrit donc **ce qui a été fait**.

> **Trois kinds construits** : `ScriptStep`, **`AgentStep`** (`D-030`) et **`TaskStep`** (`D-032`, couture Core) existent. L'agent est passé **devant** le `TaskStep` (`TaskStep` dépend d'un client tracker que la trajectoire a écarté d'ouvrir en premier), mais la couture Core du `TaskStep` est désormais posée : il agit sur le tableau via le port `ITaskTracker`, testé sur un stub. Ne reste, pour le rendre utile, que le **client Linear réel** (jambe 2·2b, projet dédié hors Core) et son **authoring UI** (2·2c).

**Comment le kind se distingue — le point que la pratique a corrigé.** Pas par un discriminant-propriété sur une `StepDefinition` restée concrète, mais par **sous-typage** : `StepDefinition` est **abstraite**, `ScriptStep`/`AgentStep` en héritent, chacun ne portant que ses propres propriétés (non-nulles). Le `kind` du document (`"agent"`, absent = script par retombée) vit **dans le JSON seulement** ; le sérialiseur, en adaptateur, construit le bon sous-type. C'est la convention de modélisation de `CLAUDE.md` — elle **supersède** la formulation « discriminant `StepKind` » ci-dessous (item 1), gardée pour mémoire.

**Ce qui a bougé :**
1. ~~`StepDefinition` — introduction d'un discriminant `StepKind`~~ → **sous-types de `StepDefinition` abstraite** (voir ci-dessus).
...
Question tranchée par la construction (`D-030`) : un `AgentStep` **headless** rend un `ScriptResult` (donc routable tel quel, `Guard` intacte). L'exécution est descendue du moteur dans un `IStepExecutor` **par type** — le moteur choisit l'exécuteur sur `CanExecute(step)` et délègue, sans connaître les kinds ; `WorkflowEngine` câble `ScriptStepExecutor` **et** `AgentStepExecutor` dès son ctor (tous deux sur le même `IProcessRunner`). Ajouter `TaskStep`, ce sera un exécuteur de plus dans cette liste, avec **son** collaborateur (le client tracker), sans toucher la boucle. La forme « résultat d'une autre nature » qui obligerait à généraliser `Guard` ne se rouvrira, s'il le faut, qu'avec la **session PTY** interactive (§2.2).
...
##### Le lien projet ↔ tableau — CONSTRUIT (2·2b·3b, `D-037`)
...
#### 7.10.4 `TaskStep` natif plutôt qu'un exécutable `cursus-task` — TRANCHÉ, COUTURE CORE CONSTRUITE (`D-032`)

> **Construit (jambe 2·2a, `D-032`) :** le `TaskStep` natif existe **dans le Core** — `TaskOperation` (`ReadTask`/`MoveCard`/`ApplyLabel`, variante de type), `TaskStepExecutor` dont le collaborateur est le port `ITaskTracker`, la clé de la tâche du run descendant par un `StepExecutionContext`. Testé contre un stub, sans réseau. Le **client réel** (Linear), lui, reste à bâtir dans son projet dédié hors Core (**2·2b**) ; l'authoring UI suit (**2·2c**). Ce qui suit est la décision de conception, inchangée.
...
- **Un quatrième projet de composition** — écarté d'abord, **retenu depuis** (`D-074`, CONSTRUIT en
  ...
  qui référence `Cursus.Core` et `Cursus.Persistence` ; `Cursus.App` le référence, jamais l'inverse —
  et `Cursus.Mcp`, **qui n'existe pas encore**, le référencera de même. ⚠️ **Le nom crée un piège de
  résolution** : depuis le namespace
...
(§9.2-15) est **construite** en 6c·3c (`PathStrategy`, `D-014`) : sa part pure est testée, et le **mécanisme
est prouvé au réel** (jambe 1 : `ProcessRunner` lançant `dotnet` nu sous un `PATH` **vidé** le résout via
`~/.asdf/shims` et sort 0 — preuve plus dure que le bundle, dont le `PATH` garde `/usr/bin`). Ne reste que
la confirmation par **double-clic sur le `.app`** installé.
...
| **Le run est la porte d'entrée d'un projet** | L'éditeur de graphe sort du chemin critique — la configuration se visite sans s'habiter, et relève d'un **engrenage** plutôt que d'un mode de même poids (maquette du 2026-07-21, `parcours.md` §1.2). ⚠️ Nuance apportée par la même passe : « l'éditeur sort du chemin critique » ne veut pas dire « rien à faire en configuration au jalon 6 ». `ValidationReport` était alors **construit, testé, et affiché nulle part** — ses problèmes ne se découvraient qu'au lancement d'un run. L'exposer était le palier 1 des trois de `parcours.md` §7. ⚠️ **Refermé depuis** : l'éditeur calcule le rapport à chaque rendu et l'affiche (`WorkflowEditorViewModel.Problems`, lié dans `MainWindow.axaml`) |
...
### 7.14 `cursus-cli` : une seconde stack, et le trousseau devenu contrat — CONSTRUIT (`D-044`, `D-045`, `D-046`)
...
Le type `CommentTarget` porte cette variante **par le type** : `{ kind: "project" | "issue", id,
label }`, là où `DocumentSummary` portait deux optionnels mutuellement exclusifs. Ces deux champs
étaient tolérables tant qu'ils décoraient l'affichage ; ils sont devenus porteurs de **routage
d'écriture**, donc l'état « les deux renseignés » a cessé d'être inoffensif. L'écart de lettre avec la
convention — l'étiquette `kind` vit dans le modèle, pas seulement dans le JSON — est assumé et motivé
en `D-046` : sans héritage, l'étiquette *est* le mécanisme de sous-typage.

Ce qui remplace l'ancrage perdu est un **repère calculé** : le titre du document, puis le titre ATX le
plus proche au-dessus du passage, en tête du corps. L'appelant ne le fournit pas, donc un agent ne peut
ni l'omettre ni le falsifier. ⚠️ Les dièses d'un **bloc de code clôturé** en sont exclus, sans quoi un
`# dotnet build …` d'un bloc `bash` serait retenu comme section — rencontré dès la première épreuve.
...
⚠️ **Le décompte des remarques ouvertes ne compte que les racines.** Mesuré : la réponse qui solde un
fil a son propre `resolvedAt` **nul**. La compter ferait que la porte du cycle de revue — *zéro
remarque ouverte* — ne se fermerait jamais, chaque solde en ajoutant une. Le piège était inoffensif
tant que la lecture portait sur un document ; il est devenu réel avec la cible.

**Registre.** Construit et éprouvé contre l'API réelle, sur les **deux genres de cible** : `login`/
`logout`, `whoami`, `doc list|show`, `comment add|list|resolve`. Non construit : le signalement des
...
1. **La moitié sessions/PTY n'est pas reliée au workflow** (§2) : aucun adaptateur entre `StartPty` et `IProcessRunner`, `SessionKind.Agent` mort. La couture elle-même est une question ouverte (§2.2, tranchée pour le *headless* `D-030`, ouverte pour le PTY interactif). ⚠️ *Rectificatif depuis 6c* : la moitié **workflow**, elle, est bel et bien câblée à l'UI — l'app charge, **lance** (`ProjectHost.LaunchAsync`), visualise et annule un run (§4.16–4.18). Ce qui manque est qu'un run s'**adosse à une session terminal vivante**, pas le lancement.
...
**Présentation et composition** (`presentation.md` §8) — quatre questions laissées ouvertes par la conception du 2026-07-21. **La surface du run est tranchée** par le parcours (`parcours.md` §4) et **construite** (§4.18) : workflows et sessions sont **deux modes**, jamais deux surfaces en compétition — **pas de routeur**, la coquille montre l'un ou l'autre. La **stratégie `PATH`** est tranchée et construite au 6c·3c (`D-014`, trou 15), **mécanisme prouvé au réel**
(jambe 1 : `dotnet` nu résolu sous `PATH` vidé) — reste la confirmation par double-clic sur le `.app`. Les deux dernières — le gel d'`INotifyPropertyChanged` dans le noyau, et le moment d'adopter un harnais headless — sont **reportées sans coût**.
